...
set sPathWork=C:\shared\Desa2ToDesa\
...
if exist !sPathWork!invert2.txt del !sPathWork!invert2.txt
	...
	for /f "tokens=1,3,5,7,9" %%u in (!sPathWork!invert.txt) do (
		if not "%%v"=="D" (rem si el segundo flag no es "D" no es una carpeta, entonces procesa la linea
			...
			if "%%u"=="PM" (
				echo movio una carpeta, tambien se debe copiar
				if "%%y"=="!ACTIVIDAD!" (
					for /f "tokens=1 delims=;" %%m in ("%%x") do (
...
set tFlag_folder=flag_desconocido
set inv3_field2=
...
rem 							echo u==sTemp: !sTemp!
							...
							rem si se trata de una fila del tipo: ON, CO, NN no la registra en el archivo de salida, la ignora.
							if not %%t==ON if not %%t==CO if not %%t==NN (
